  ...
  async create(driverData: CreateDriverInput): Promise<Driver> {
    ...
    try {
      ...
      const driverResult = await query(
        `INSERT INTO drivers (
          first_name,last_name,full_name, phone_number, email, profile_pic_url,date_of_birth, gender, 
          address, role, status, kyc, credit , performance, payments
        ...
        [
          ...
          //   ? JSON.stringify(driverData.availability)
          //   : '{"online": false, "lastActive": null}',
  ...
  async update(id: string, driverData: UpdateDriverInput): Promise<Driver | null> {
    ...
    try {
      ...
      if (driverData.date_of_birth) {
        driverFields.push(`dob = $${paramCount++}`);
      ...
      if (driverData.vehicle) {
        ...
        if (vehicleCheck.rows.length > 0) {
          ...
          if (driverData.vehicle.vehicleNumber) {
            vehicleFields.push(`vehicle_number = $${vParamCount++}`);
            vehicleValues.push(driverData.vehicle.vehicleNumber);
          }
          if (driverData.vehicle.vehicleModel) {
            vehicleFields.push(`vehicle_model = $${vParamCount++}`);
            vehicleValues.push(driverData.vehicle.vehicleModel);
          }
          if (driverData.vehicle.vehicleType) {
            vehicleFields.push(`vehicle_type = $${vParamCount++}`);
            vehicleValues.push(driverData.vehicle.vehicleType);
          }
          if (driverData.vehicle.fuelType) {
            vehicleFields.push(`fuel_type = $${vParamCount++}`);
            vehicleValues.push(driverData.vehicle.fuelType);
          }
          if (driverData.vehicle.registrationDate) {
            vehicleFields.push(`registration_date = $${vParamCount++}`);
            vehicleValues.push(driverData.vehicle.registrationDate);
          }
          if (driverData.vehicle.insuranceExpiry) {
            vehicleFields.push(`insurance_expiry = $${vParamCount++}`);
            vehicleValues.push(driverData.vehicle.insuranceExpiry);
          }
          if (driverData.vehicle.rcDocumentUrl) {
            vehicleFields.push(`rc_document_url = $${vParamCount++}`);
            vehicleValues.push(driverData.vehicle.rcDocumentUrl);
          }
          if (driverData.vehicle.status !== undefined) {
            vehicleFields.push(`status = $${vParamCount++}`);
            vehicleValues.push(driverData.vehicle.status);
          }
        ...
        for (const doc of driverData.documents) {
          ...
          if (docIdToUpdate) {
            ...
            if (doc.documentUrl) {
              ...
              docValues.push(doc.documentUrl);
            }
