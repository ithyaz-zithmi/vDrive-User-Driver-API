...
export class SosController {
  static async triggerSos(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const driver_id = (req as any).user.id; // From auth middleware

      const sosEvent = await SosService.triggerSos(driver_id, trip_id);
  ...
  static async addTrustedContact(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const contact = await SosRepository.addTrustedContact(driver_id, name, phone);
  ...
  static async removeTrustedContact(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await SosRepository.removeTrustedContact(Number(id), driver_id);
